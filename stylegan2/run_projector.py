...
from stylegan2.dnnlib import submit_config, EasyDict, SubmitConfig, submit_run, SubmitTarget, make_run_dir_path
import re
import sys
...
def project_generated_images(network_pkl, seeds, num_snapshots, truncation_psi):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        project_image(proj, targets=images, png_prefix=make_run_dir_path('seed%04d-' % seed), num_snapshots=num_snapshots)
...
def project_real_images(network_pkl, dataset_name, data_dir, num_images, num_snapshots):
    ...
    for image_idx in range(num_images):
        ...
        project_image(proj, targets=images, png_prefix=make_run_dir_path('image%04d-' % image_idx), num_snapshots=num_snapshots)
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    submit_run(sc, func_name_map[subcmd], **kwargs)
