...
class CocoWallFeatureDetectionConfig(Config):
    """Configuration for training on the toy shapes dataset.
        Derives from the base Config class and overrides values specific
        to the toy shapes dataset.
        """
    # Give the configuration a recognizable name
    NAME = "wall_feature_detection"
    ...
    NUM_CLASSES = 1 + 4  # background + brick_door + brick_window + white_door + white_window
    ...
    RPN_ANCHOR_SCALES = (64, 80, 128, 160, 256)  # anchor side in pixels
...
#config = CocoWallFeatureDetectionConfig()
#config.display()


class CocoWallFeatureDetectionDataset(utils.Dataset):
    ...
    def load_wall_feature_detection(self, base_dir, subset):
        print(base_dir)
        print(subset)
        # Add classes
        self.add_class("wall_feature_detection", 1, "white_window")
        self.add_class("wall_feature_detection", 2, "white_door")
        self.add_class("wall_feature_detection", 3, "brick_window")
        self.add_class("wall_feature_detection", 4, "brick_door")
        ...
        for annotation, counter in zip(annotations, range(0, len(annotations))):
            ...
            class_id_sequence = np.zeros(len(picture_annotation["regions"]), dtype=np.uint32)
            for region, i in zip(picture_annotation["regions"], range(len(picture_annotation["regions"]))):
                #print(list(region["region_attributes"]["wall_features"].keys())[0])
                print(list(filter(lambda x: x["name"] == list(region["region_attributes"]["wall_features"].keys())[0], self.class_info))[0]["id"])
                class_id_sequence[i] = (list(filter(lambda x: x["name"] == list(region["region_attributes"]["wall_features"].keys())[0], self.class_info))[0]["id"])
            ...
            self.id_picname_mask_map.append({"id": counter, "picname": picture, "mask": mask, "bool_mask": bool_mask, "class_id_sequence": class_id_sequence})
            self.add_image("wall_feature_detection", image_id=counter, path=os.path.join(dataset_dir, picture_annotation["filename"]), picname=picture_annotation["filename"])
...
"""
# Training dataset
dataset_train = CocoWallFeatureDetectionDataset()
dataset_train.load_wall_feature_detection("/home/bernihoh/Bachelor/SMS/MaskRCNN/samples/SMSNetworks/wall_feature_place_detection/", "train")
dataset_train.prepare()

# Validation dataset
dataset_val = CocoWallFeatureDetectionDataset()
...
dataset_val.load_wall_feature_detection("/home/bernihoh/Bachelor/SMS/MaskRCNN/samples/SMSNetworks/wall_feature_place_detection/", "val")
dataset_val.prepare()
...
if init_with == "imagenet":
    ...
    model.load_weights("/home/bernihoh/Bachelor/SMS/MaskRCNN/samples/SMSNetworks/wall_feature_place_detection/mask_rcnn_head detection_0020.h5")
...
class InferenceConfig(CocoWallFeatureDetectionConfig):
    GPU_COUNT = 1
    IMAGES_PER_GPU = 1
